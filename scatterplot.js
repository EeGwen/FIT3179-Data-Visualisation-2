document.addEventListener('DOMContentLoaded', function () {
  ...
  d3.csv(DATA_URL).then(raw => {
    ...
    function renderFor(selection){
      ...
      noteEl.textContent = lr ? `Regression (approx): ${key} = ${lr.slope.toFixed(4)} × AQI + ${lr.intercept.toFixed(4)}` : `Data loaded: ${displayRows.length} rows`;
      ...
      fetch('js/scatterplot.vg.json')
        ...
        .then(spec=>{
          ...
          vegaEmbed(containerId, spec, {actions:true, renderer:"canvas"})
            .catch(err=>{ console.error(err); document.querySelector(containerId).innerHTML="<p style='color:#666;'>Failed to render chart.</p>"; });
        });
    }

    // radio buttons
    const radios = document.querySelectorAll('input[name="caseType"]');
    radios.forEach(r=>r.addEventListener('change', ()=>renderFor(document.querySelector('input[name="caseType"]:checked').value)));
